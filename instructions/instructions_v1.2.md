...
2. User can pick local files/folder and see a folder tree in the left pane.
3. Folder nodes can be expanded and collapsed.
4. Only folders and CSV files are shown in the tree.
5. Double-clicking a CSV opens it in the main grid.
6. Grid displays visible row/column boundaries.
7. Double-clicking a cell enables editing and updates value.
8. Add/Remove Row buttons correctly change row count.
9. Add/Remove Column buttons correctly change column count.
10. Save writes updates to the original file when possible.
11. Save As exports current data to a new CSV file.
12. New File creates an editable new CSV document.
13. Unsaved-change warning appears before destructive navigation actions.
...
In the next step, generate or update the full React app code in `./app` according to this specification. Include any needed dependencies and scripts so the project runs with `npm start`.
